        ...
        while not self.task_queue.empty() or not self.stop_event.is_set():
            ...
            try:
                ...
                if (['gpu'] == []):
                    ...
                    jobdir="{Name}_{Algorithm}_{Struct}_{Layer}_{Loss}_{LR}_{IC}_{M}_{N}_{G}_{E}_{Epochs}_{Note}".format(
                    ...
                    with open(job["Log Name"],"a+") as f:
                        ...
                        f.write("{M},{N},{t},{G},{E},{W},{aimPSL},{Algorithm},{Struct},{Layer},{Loss},{LR},{Epochs},{IC},{PC},".format(
                                                                                                M = job['M'],
                                                                                                N = job['N'],
                                                                                                t = job['t'],
                                                                                                G = job['G'],
...
if __name__ == "__main__":
    if (len(sys.argv) == 1):
        ...
        with open("example.csv", "w", newline='') as csvfile:
            ...
            writer.writerow(["M","N","t","G","E","W","aimPSL","Algorithm","Struct","Layer","Loss","Learning Rate","Epochs","Initial Code","Opt Func","Eva Func","Result","Log Name"])
            writer.writerow(["4","32",float(0.01/4/32),"0","31","0.999","-14","NN","NNGD","1","NNGDLOSS","lrstepscheduler","20000","c","TrainNN","CalSL","0","0"])
            writer.writerow(["4","64",float(0.01/4/64),"0","63","0.999","-18","NN","NNGD","1","NNGDLOSS","lrstepscheduler","20000","c","TrainNN","CalSL","0","0"])
            writer.writerow(["4","256",float(0.01/4/256),"0","255","0.999","-24","NN","NNGD","1","NNGDLOSS","lrstepscheduler","20000","c","TrainNN","CalSL","0","0"])
            writer.writerow(["4","1024",float(0.01/4/1024),"0","1023","0.999","-30","NN","NNGD","1","NNGDLOSS","lrstepscheduler","20000","c","TrainNN","CalSL","0","0"])
            writer.writerow(["10","1024",float(0.01/10/256),"0","1023","0.999","-28","NN","NNGD","1","NNGDLOSS","lrstepscheduler","20000","c","TrainNN","CalSL","0","0"])
            writer.writerow(["128","1024",float(0.01/128/256),"0","1023","0.999","-25","NN","NNGD","1","NNGDLOSS","lrstepscheduler","20000","c","TrainNN","CalSL","0","0"])
    ...
    for job in joblist:
        ...
        job["E"] = int(job["E"])
        job["W"] = float(job["W"])
        job["aimPSL"] = float(job["aimPSL"])
        job["Layer"] = int(job["Layer"])
        job["Epochs"] = int(job["Epochs"])
        if 'Note' not in job.keys():
            job["Note"] = ""
    ...
    LogList = ["Log Time", "Duration", "M", "N", "t", "G", "E", "W", "aimPSL", "Algorithm", "Struct", "Layer", "Loss", "Learnning Rate", "Epochs", "Initial Code", "Params Count"]
