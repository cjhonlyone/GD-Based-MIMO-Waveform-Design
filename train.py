...
def TrainNN(job):
    ...
    for t in range(epochs):
        ...
        if 10*torch.log10(PSL) < job['aimPSL']:
            break
    ...
    job["Result"] = {"PSL":10*torch.log10(PSL),"ISL":10*torch.log10(ISL),"APSL":APSL,"AISL":AISL,"CPSL":CPSL,"CISL":CISL,"time":end-start}
...
def LogSL(job, waveform):
    ...
    job["Result"]["APSL"] = APSL
    job["Result"]["AISL"] = AISL
    job["Result"]["CPSL"] = CPSL
    job["Result"]["CISL"] = CISL